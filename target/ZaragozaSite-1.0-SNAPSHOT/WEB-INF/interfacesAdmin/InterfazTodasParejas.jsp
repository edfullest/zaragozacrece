                ...
                <div class="body"> 
                    ...
                    <div class="sky-tabs sky-tabs-pos-top-left sky-tabs-response-to-icons">
                        <div class="textbox">
                            <section>
                                ...
                                <ul>
                                    <li>Este apartado es para asignar pagos a parejas. Sólo procede si algún padrino de la pareja pagó 375 pesos.</li>
                                    <li>Si su correo aparece en <b><font color="red">Rojo</font></b>, significa que aún no se ha registrado</li>
                                </ul>
                            </section>
                        </div>
                    </div>
                    <div class="component">
                        ...
                        <table id="tabla">
                            ...
                            <c:forEach items="${parejas}" var="item" >
                                <tr>
                                    <td><c:out value="${item.idPareja}"/>
                                    </td>
                                    <td><c:out value="${item.correo1}"/>
                                    </td>
                                    
                                    <c:choose>
                                        <c:when test="${item.idPadrino2==-1}">
                                        <td><font color="red"><c:out value="${item.correo2}"/></font>
                                            </td>
                                        </c:when>
                                        <c:otherwise>
                                            <td><c:out value="${item.correo2}"/>
                                    </td>
                                        </c:otherwise>
                                    </c:choose>
                                    ...
                                    <td><a href="cargarPagosParejas?tipo=redirigirSeleccionarPadrino&idPareja=${item.idPareja}&idPadrino1=${item.idPadrino1}&idPadrino2=${item.idPadrino2}&correo1=${item.correo1}&correo2=${item.correo2}">
                                            <button type="button" class="button-table"><i class="fa fa-caret-square-o-right"></i> Seleccionar Pareja</button></a></td>
                            ...
                            <nav role="navigation"> 
                                <ul class="cd-pagination">
                                    ...
                                    <c:if test="${paginaActual != 1}">
                                        <li><a href="cargarPagosParejas?tipo=cargarTodasParejas&paginaActual=${paginaActual - 1}">Anterior</a></li>
                                        </c:if>
                                    ...
                                    <c:forEach begin="1" end="${numPaginas}" var="i">
                                        <c:choose>
                                            <c:when test="${paginaActual eq i}">
                                                ...
                                                <c:otherwise>
                                                <li><a href="cargarPagosParejas?tipo=cargarTodasParejas&paginaActual=${i}">${i}</a></li>
                                                </c:otherwise>
                                            </c:choose>
                                        </c:forEach>
                                    
                                    <c:if test="${paginaActual != numPaginas}">
                                        <li><a href="cargarPagosParejas?tipo=cargarTodasParejas&paginaActual=${paginaActual + 1}">Siguiente</a></li>
                                        </c:if>
