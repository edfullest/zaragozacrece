    ...
    <c:choose>
		...
		<div class="body">
			...
			<!-- tabs -->
				...
				<input type="radio" name="opcion" value="2" onclick="submit()" checked id="sky-tab2" class="sky-tab-content-2">
						...
						<div class="typography">
                                                     <c:choose>
                                                         <c:when test="${respuesta.equals('esperandoAdmin')}">
                                                            <font size="4"> El pago más reciente hecho por ti y <b>${correo}</b> ya está registrado.
                                                             Sólo falta que el administrador les asigne un apadrinado, o bien, si ya tienen un apadrinado, que les renueve su suscripcion! </font>
                                                             
                                                         </c:when>
                                                         <c:when test="${respuesta.equals('asignado')}">
                                                             <font size="4">Tú y <b>${correo}</b> tienen su pago asignado a una suscripcion.
                                                             Mantente informado acerca del próximo viaje, porque la suscripción acaba días antes del mismo! </font>
                                                             
                                                         </c:when>
                                                          <c:when test="${respuesta.equals('noPagaste')}">
                                                              <font size="4">No has pagado y <b>${correo}</b> ya pagó!
                                                              Para pagar con PayPal, haga click en el botón "Pagar":</font>
                                                             <div style="text-align:center;">
                                                              <a  href="ControlPanelPadrino?paypal=${"redirigirPaypal"}"> 
                                                         ...
                                                         <c:when test="${respuesta.equals('nadieHaPagado')}">
                                                             <font size="4">No has pagado ni tú ni <b>${correo}</b>
                                                             Para pagar con PayPal, haga click en el botón "Pagar":</font>
                                                             <a href="ControlPanelPadrino?paypal=${"redirigirPaypal"}"><button type="button" class="button">Pagar</button></a>
                                                                                                     
                                                             
                                         
                                                         </c:when>
                                                          <c:when test="${respuesta.equals('sinRegistro')}">
                                                             <font size="4"><b>${correo}</b> no se ha registrado aún! Invítalo a que se registre! </font>
